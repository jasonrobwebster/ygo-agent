...
*   **Refactored**: The logic has been fully moved to `YgoCoreAdapter`, and `YGOProEnv` now delegates calls to the adapter.
...
| :--- | :--- | :--- | :--- | :--- |
| `combinations()` | `ygopro.h` | `utils.h` | General-purpose combination utility. | Pending |
| `sum_to()` | `ygopro.h` | `utils.h` | Helper for `combinations_with_weight`. | Pending |
| `combinations_with_weight()` | `ygopro.h` | `utils.h` | Used for MSG_SELECT_SUM. | Pending |
| `msg_to_string()` | `ygopro.h` | `utils.h` | Debugging utility. | Pending |
| `ls_to_spec()`, `spec_to_ls()` | `ygopro.h` | `utils.h` | Location/spec string conversion. | Pending |
| `read_decks()` | `ygopro.h` | `utils.h` | Deck reading utility. | Pending |
| `LegalAction` struct | `ygopro.h` | `ygo_data_types.h` | Becomes a core data structure. | Pending |
| `Card` class | `ygopro.h` | `ygo_data_types.h` | Becomes a core data structure. | Pending |
| `init_module()` | `ygopro.h` | `YgoCoreAdapter` | Becomes a static method on the adapter to initialize shared resources. | Pending |
